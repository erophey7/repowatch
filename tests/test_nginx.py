...
def test_prefetch_marker_omitted_when_syslog_listener_disabled():
    # syslog_listener.enabled defaults to True since 2026-09-14 — must be
...
def test_nginx_config_enable_cache_probe_defaults_to_false_and_validates_type():
    ...
    assert NginxConfig(enable_cache_probe=True).enable_cache_probe is True
    with pytest.raises(ConfigError, match='enable_cache_probe'):
        NginxConfig(enable_cache_probe='yes')
...
def test_render_probe_js_embeds_cache_dir_and_both_handlers():
    ...
    assert 'function scan(r) {' in text
    assert 'export default {probe, scan};' in text
    # levels=1:2 split — must match proxy_cache_path's hardcoded scheme in render().
    assert "hash.slice(-1) + '/' + hash.slice(-3, -1) + '/' + hash" in text
...
def test_render_probe_conf_renders_loopback_only_locations():
    ...
    assert 'js_content cache_probe.scan;' in text
    assert text.count('allow 127.0.0.1;') == 2
    assert text.count('deny all;') == 2
    # /purge-raw lives in render_purge(), not here — see its own tests below.
    assert 'purge-raw' not in text
...
def test_purge_raw_location_requires_both_enable_purge_and_enable_cache_probe():
    ...
    assert 'allow 127.0.0.1;' in purge_raw_block
    assert 'deny all;' in purge_raw_block


def test_compute_cache_key_matches_render_purge_basis_when_dedup_off():
    """compute_cache_key() must produce the exact same string render_purge()
    builds via nginx variables at request time — same guard against the
...
def test_compute_cache_key_matches_render_purge_basis_when_dedup_on():
    """Same real bug class as test_render_purge_key_matches_dedup_uri_basis_
    when_dedup_is_also_enabled — a pacman repo where local != remote makes
    the distinction unambiguous."""
    pacman_repo = RepoConfig('core', 'pacman', 'https://mirror.test/core/os/x86_64', 'x86_64', repo_name='core')
    ...
    assert '/arch/core/os/x86_64/bash-1-x86_64.pkg.tar.zst' not in key


def test_compute_cache_key_raises_for_a_repo_not_in_the_configs_routes():
    c = config([apt()])
...
def test_apply_writes_probe_files_and_includes_them_when_enabled(tmp_path, monkeypatch):
    ...
    active = tmp_path / 'active.conf'
    assert f'include {probe_conf_path};' in active.read_text()
    assert f'js_import cache_probe from {probe_js_path};' in active.read_text()
...
def test_apply_rolls_back_probe_files_together_with_active_conf(tmp_path, monkeypatch):
    ...
    probe_js_path = tmp_path / 'probe.js'
    active.write_text('# previous working\n')
    probe_conf_path.write_text('# previous probe conf\n')
    probe_js_path.write_text('// previous probe js\n')
    calls = []
    def run(args, **kw):
        calls.append(args)
        if len(calls) == 1:
...
def test_apply_detects_a_probe_only_content_change_not_just_active_conf(tmp_path, monkeypatch):
    """Regression guard: the no-op comparison must include the probe files
    too, or toggling enable_cache_probe alone (active.conf's OWN content
    also changes here via js_import/include, but a hypothetical future
