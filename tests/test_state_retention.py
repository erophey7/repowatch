...
def _insert_event(store: StateStore, repo_id: str, ts: datetime) -> None:
...
def test_prune_events_removes_only_old_rows(tmp_path):
    ...
    removed = store.prune_events(retention_days=90)

    assert removed == 1
    remaining = store.get_history("repo-a", limit=10)
...
def test_prune_events_noop_when_nothing_old(tmp_path):
    ...
    assert removed == 0
    assert len(store.get_history("repo-a", limit=10)) == 1


def _insert_warmed(store: StateStore, repo_id: str, package_key: str, warmed_at: datetime) -> None:
    with sqlite3.connect(store.db_path) as conn:
        conn.execute(
...
def test_prune_warmed_packages_removes_only_old_rows(tmp_path):
    ...
    removed = store.prune_warmed_packages(retention_days=180)

    assert removed == 1
    remaining = store.get_warmed_packages("repo-a")
...
def test_prune_events_by_size_keeps_only_n_most_recent_per_repo(tmp_path):
    ...
    removed = store.prune_events_by_size(max_rows_per_repo=2)
...
def test_prune_events_by_size_is_per_repo_not_global(tmp_path):
    ...
    assert removed == 1
    assert len(store.get_history("repo-busy", limit=10)) == 2
    assert len(store.get_history("repo-quiet", limit=10)) == 1
...
def test_prune_events_by_size_noop_when_under_limit(tmp_path):
    ...
    removed = store.prune_events_by_size(max_rows_per_repo=10)

    assert removed == 0
    assert len(store.get_history("repo-a", limit=10)) == 1


def _insert_request(store: StateStore, repo_id: str | None, ts: datetime) -> None:
    with sqlite3.connect(store.db_path) as conn:
        conn.execute(
...
def test_prune_requests_by_size_keeps_n_most_recent_globally(tmp_path):
    ...
    # limit is global, per-repo doesn't apply here (see StateStore.prune_requests_by_size)
    ...
    removed = store.prune_requests_by_size(max_rows=2)

    assert removed == 2
    remaining = store.get_recent_requests(limit=10)
    assert len(remaining) == 2
    assert remaining[0]["ts"] == (now - timedelta(minutes=0)).isoformat(timespec="seconds")
    assert remaining[1]["ts"] == (now - timedelta(minutes=1)).isoformat(timespec="seconds")
...
def test_prune_requests_by_size_noop_when_under_limit(tmp_path):
    ...
    removed = store.prune_requests_by_size(max_rows=10)

    assert removed == 0
    assert len(store.get_recent_requests(limit=10)) == 1
