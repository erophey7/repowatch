...
def test_replacement_diff_and_history(tmp_path, old_file, new_file, old_hash, new_hash, modified):
    ...
    store.repositories.record_snapshot(snapshot(old_file, old_hash))
    diff = store.repositories.record_snapshot(snapshot(new_file, new_hash))
    ...
    assert bool(store.cache.get_pending_replacements('r')) is modified
    if modified:
        assert store.repositories.get_history('r')[0]['modified_packages'] == ['foo-1']
        assert store.repositories.get_status('r')['last_modified_packages'] == ['foo-1']


def test_pending_survives_restart_and_new_replacement_preserves_old_targets(tmp_path):
    path = tmp_path / 'state'
    store = ServiceState(path)
    store.repositories.record_snapshot(snapshot('a.rpm'))
    store.repositories.record_snapshot(snapshot('b.rpm'))
    old = store.cache.get_pending_replacements('r')[0]
    store = ServiceState(path)
    assert store.cache.get_pending_replacements('r')[0] == old
    store.repositories.record_snapshot(snapshot('c.rpm'))
    current = store.cache.get_pending_replacements('r')[0]
    assert set(map(tuple, current['targets'])) == {('r', 'a.rpm'), ('r', 'b.rpm'), ('r', 'c.rpm')}
    store.cache.finish_replacement('r', 'foo-1', old['revision'])
    assert store.cache.get_pending_replacements('r')
    store.repositories.record_snapshot(RepoSnapshot('r'))
    # Removal must not discard unpurged historical filenames.
    assert store.cache.get_pending_replacements('r')[0]['filename'] is None
...
def test_old_event_schema_migrates_without_losing_history(tmp_path):
    ...
    store = ServiceState(path)
    assert store.repositories.get_history('r')[0]['modified_packages'] == []
    assert store.repositories.get_history('r')[0]['new_packages'] == ['foo-1']
...
def test_watcher_replacement_purges_before_warming_and_retries_on_unchanged_head(
    ...
    asyncio.run(operations_check.check_repo(config, repo, store))
    ...
    assert not store.cache.get_pending_replacements('r')
...
def test_disabled_purge_exposes_pending_without_warming_old_hit(tmp_path, monkeypatch):
    store = ServiceState(tmp_path / 'state')
    store.repositories.record_snapshot(snapshot('old.rpm'))
    store.repositories.record_snapshot(snapshot('new.rpm'))
    repo = RepoConfig('r', 'dnf', 'https://mirror.test/repo', 'x86_64')
    config = Config(tmp_path / 'state', 300, 'http://cache.test', StatusServerConfig(), repos=[repo])
    warm = AsyncMock()
    monkeypatch.setattr(operations_check, 'warm_cache', warm)
    monkeypatch.setattr(operations_replacements, 'warm_cache', warm)
    asyncio.run(operations_replacements.refresh_replacements(config, repo, store))
    warm.assert_not_called()
    assert 'enable_purge' in store.cache.get_pending_replacements('r')[0]['last_error']
...
def test_replacement_invalidates_without_overriding_warming_policy(tmp_path, monkeypatch, policy):
    ...
    monkeypatch.setattr(operations_replacements, 'purge_selected', purge)
    warm = AsyncMock()
    monkeypatch.setattr(operations_check, 'warm_cache', warm)
    monkeypatch.setattr(operations_replacements, 'warm_cache', warm)
    asyncio.run(operations_replacements.refresh_replacements(config, repo, store))
...
def test_pending_replacement_keeps_known_hash_if_metadata_temporarily_loses_it(tmp_path):
    store = ServiceState(tmp_path / 'state')
    store.repositories.record_snapshot(snapshot(digest='a' * 64))
    store.repositories.record_snapshot(snapshot(digest='b' * 64))
    diff = store.repositories.record_snapshot(snapshot())
    assert not diff.changed
    assert store.cache.get_pending_replacements('r')[0]['content_hash'] == 'b' * 64


def test_replacement_preserves_previous_canonical_location(tmp_path):
    store = ServiceState(tmp_path / 'state')
    store.repositories.record_snapshot(RepoSnapshot('a', {'foo-1': 'foo.rpm'}, content_hashes={'foo-1': 'a' * 64}))
    store.repositories.record_snapshot(snapshot(digest='a' * 64))
    store.repositories.record_snapshot(snapshot(digest='b' * 64))
    assert ['a', 'foo.rpm'] in store.cache.get_pending_replacements('r')[0]['targets']
