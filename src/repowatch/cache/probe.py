...
same convention as cache.purge.purge_removed's own enable_purge check.

/cache-probe is a non-destructive complement to cache.purge.purge_selected():
...
# (nginx/render.py's render()) can ever produce: 16 possible 1-char first levels x
...
# via /cache-scan. Kept in this module (not nginx/render.py) since it's a
...
class Inventory:
    ...
    # reporting.statistics.cache_dir_stats()'s `inaccessible_directories` already exists to
...
async def probe(client: httpx.AsyncClient, base_url: str, key: str) -> ProbeResult:
    ...
    string — see routing.compute_cache_key() to build one for a package.

    Safe to use httpx's dict-based `params={"key": key}` here (unlike
    purge_raw()) — this hits njs's r.args.key (nginx.render.render_probe_js()'s
...
async def scan_leaf(client: httpx.AsyncClient, base_url: str, leaf: str) -> list[ScanEntry]:
    ...
    the whole cache at once (see nginx.render.render_probe_js()'s docstring)."""
    resp = await client.get(
...
async def cache_dir_size(base_url: str, *, concurrency: int = 8) -> dict:
    ...
    async with httpx.AsyncClient() as client:
        ...
        # pitfall reporting.statistics.cache_dir_stats() already guards against for
...
async def purge_selected_raw(
    ...
    *, canonical_keys: dict[str, str] | None = None,
    ...
    """Drop-in alternative to cache.purge.purge_selected() — same inputs
    ({package_key: filename}), same result contract per key
    ("purged"/"not_cached"/"error (...)") — used by web/handler.py's dashboard
    purge handlers (purge_selected_payload, remove_warmed_package_payload)
    INSTEAD of cache.purge.purge_selected() whenever nginx.enable_cache_probe
    is on.

    Goes through the route-independent /purge-raw location (see
    nginx.render.render_purge()) instead of the per-repo /purge<prefix> one, using
    routing.compute_cache_key() to build each item's exact literal key in
