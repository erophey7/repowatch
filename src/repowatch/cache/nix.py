...
async def warm(config, repo, store, packages: dict[str, str], force: bool) -> dict[str, bool]:
    ...
    if not force and not repo.prefetch:
        return {}
    store.cache.update_nix_trust(repo.id, repo.verify_signature, repo.nix_public_keys)
        ...
        for key, filename in packages.items():
            ...
            try:
                if cache_info_ok is None:
                    cache_info_ok, _ = await download_package(client, warm_url(config, repo, 'nix-cache-info'), limiter)
                ...
                store.cache.record_nix_artifacts(repo.id, key, artifacts)
                ...
                async def download(artifact):
                    ...
                    async with semaphore:
                        ...
                        if not result[0] and result[1] == 200 and config.nginx.enable_purge:
                            ...
                            from repowatch.cache.probe import purge_selected_raw
...
async def purge(config, repo, store, packages: dict[str, str]) -> dict[str, str]:
    ...
    shared = store.cache.nix_shared_files(repo.id, list(packages))
    unknown_owners = store.cache.nix_has_unknown_owners(repo.id, list(packages))
    current = store.repositories.get_packages(repo.id) or {}
    shared.update(filename for key, filename in current.items() if key not in packages)
    selections = {}
    retained = {}
    files = {}
    for key, filename in packages.items():
        names = {filename} | {a['filename'] for a in store.cache.get_nix_artifacts(repo.id, key)}
        if unknown_owners:
            shared.update(names)
