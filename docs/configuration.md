...
| `request_max_rows` | int or `null` | `null` | Size-based cap on `request_events`, global (not per-repository — some rows aren't attributable to one repository). `null` = no cap. The charts above follow the cap too. |
